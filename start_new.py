...
CHATGPT_MESSAGE = "A nature documentary about * written by David Attenborough"
...
class Control:
    ...
    def harvest_chatgpt():
        while True:
            i = len(organisms)
            keyword = organisms[random.randint(0, i-1)]
    ...
    def run(self):        
        if HARVESTER_ON:
            ...
            p = multiprocessing.Process(target = harvest_chatgpt)
        while True:        
            ...
            while True:
                try:
                    ...
                    while not videosSearch:
                        videosSearch = VideosSearch(self.keyword, limit = 20) # limit = max number of returned videos
                        time.sleep(0.5)       
